...
if (!!document.getElementById('savantEditor')) {
  ...
  const core = {
    ...
    attrUpdate({ target }, data = false) {
      ...
      if (attr === 'file') {
        document.querySelector('.file div span').textContent = getFileName(
          value
        );
      }
    },
...
if (!!document.querySelector('#savantSettings')) {
  wp.element.render(
    <SavantSettings />,
    document.getElementById('savantSettings')
  );
}
